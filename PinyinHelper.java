...
public class PinyinHelper {
   ...
   * 对结果进行正则匹配处理
   * @param str 要转换拼音的字符串
   * @param outputFormat 格式化对象
   * @param separate 分隔符
   * @param retain 是否保留其它字符，true保留
   * @param byRegex 当指定有正则匹配文件时
   * @throws IOException 
   */
  static public String toHanYuPinyinString(String str, HanyuPinyinOutputFormat outputFormat,
	      String separate, boolean retain, boolean byRegex) throws BadHanyuPinyinOutputFormatCombination {
	  String regexStr = Trie.regexText(str, separate);
	  String firstPYStr = toHanYuPinyinString(regexStr, outputFormat, separate, true);
	  
	  // 排除其它字符处理
	  if (retain == false) {
		  String excludeStr = firstPYStr.replaceAll("[A-Za-z0-9]"+(separate!=null&&separate!=""?"|"+separate:""), "");
		  char [] c_arr = excludeStr.toCharArray();
		  StringBuffer sBuffer = new StringBuffer();
		  for (char c : c_arr) {
			if (Pattern.matches("\\.|\\^|\\$|\\[|\\]|\\{|\\}|\\+|\\?|\\*|\\(|\\)|\\-", String.valueOf(c))) {
				//sBuffer.append("\\").append(c);
			} else {
				if ("".equals(sBuffer.toString()) == false) {
					sBuffer.append("|");
				}
				sBuffer.append(c);
			}
		  }
		  return firstPYStr.replaceAll(sBuffer.toString(), "");
	  }
	  return firstPYStr;
  }
